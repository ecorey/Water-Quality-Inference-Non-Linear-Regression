"""
This script loads P90 score data from 2013-2023 and visualizes the GM values for 5 stations over time.
"""
...
# get 5 stations to visualize the GM values over time
...
# create a scatter plot for the GM values across years
...
plt.axhline(y=88, color='black', linestyle='--', alpha=0.5, zorder=1)
...
for i, station in enumerate(five_stations):
    ...
    plt.scatter(station_data['Year'], station_data['GM'], 
                color=colors[i], label=station, alpha=0.7, s=80)
    
    # add connecting lines for each station
    plt.plot(station_data['Year'], station_data['GM'], 
             color=colors[i], alpha=0.5, linestyle='-')

# add labels and a title
plt.xlabel('Year', fontsize=14)
plt.ylabel('GM Value', fontsize=14)
plt.title('GM Values for 5 Stations (2013-2023)', fontsize=16)
...
# save the figure
plt.tight_layout()
plt.savefig('gm_station_visualization.png', dpi=300)
...
print("\nGM values by station and year:")
pivot_table = selected_data.pivot_table(values='GM', index='Station', columns='Year', aggfunc='mean')
print(pivot_table)