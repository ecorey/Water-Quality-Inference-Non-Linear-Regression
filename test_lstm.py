...
def train_and_predict(train_data, stations, station_coords, train_up_to_year, predict_for_year, 
    ...
    """
    Train LSTM models and make predictions for each station.
    """
    import signal
    ...
    for station in stations:
        ...
        try:
            # Use a timeout of 90 seconds per station (LSTM may need more time)
            with time_limit(90):
                ...
                optimizer = torch.optim.Adam(model.parameters(), lr=0.005)  # Changed to Adam for LSTM
                
                # Training loop
                epochs = 75  # Increased for better LSTM training
                for epoch in range(epochs):
                    ...
                    # Print progress every 20 epochs for first station only
                    if epoch % 20 == 0 and station == stations[0]:
    ...
    print(f"Successful predictions with LSTM network: {total_stations - len(limited_data_stations) - len(error_stations) - len(timeout_stations)}")
...
class StationP90Model(nn.Module):
    def __init__(self, input_size):
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.lstm_hidden_size, 16),
            nn.Tanh(),
            nn.Linear(16, 1)
        )
    ...
    def forward(self, x):
        ...
        # Since we're processing one sample at a time, we need to add sequence dimension
        batch_size = x.size(0)
